jQuery(document).ready(function($){
    ...
    var getProductsAPI = function(prod, pos){
        ...
        $.get(url, function(data){
            ...
            if (pos >= prod.length)
            {
                ...
                var htmlexp = '<div class="results-wait-screen"><img src="../images/logo.png"  class="wait-logo"><h2>Results:</h2>';
                htmlexp = htmlexp + '<ul><li><b class="number-bluelight">'+$('.counter>b').first().text()+'</b> product(s) identified on '+storename+'</li>';
                htmlexp = htmlexp + '<li><b class="number-bluelight">'+(prod.length-1)+'</b> had an UPC code (required to try to match on Amazon)</li>';

                htmlexp = htmlexp + '<li>from <b class="number-bluelight">'+(prod.length-1)+'</b> product(s) with an UPC code, <b class="number-bluelight">'+((prod.length-1)-cntfinalListOfAsins)+'</b> is(are) not registered on Amazon</li>';

                htmlexp = htmlexp + '<li><span class="bigger-result"><b class="number-bluelight">'+cntfinalListOfAsins+'</b> had an exact match on Amazon!</span></li></ul>';
    ...
    var getWalmartProducts = function(html){
        ...
        html.find('.product-title-link').each(function(){
            var id = $(this).attr("href");

            var url = 'http://www.walmart.'+getCountry(activeTab) + id;
            wmproducts.push(url);
        });
    ...
    var getUPCProductPage = function(url, source, productName){
        ...
        whereIsUPC['wayfair'] = 'h1.ProductDetailInfoBlock-header';
    ...
    var getThirdStoreProducts = function (url, source){
        ...
        oneProductPage['wayfair'] = 'span.ProductDetailInfoBlock-header-title';
        oneProductPage['kohls'] = 'h1.title.productTitleName';
        oneProductPage['target'] = 'h2.title-product span';
        oneProductPage['toysrus'] = '#lTitle>h1';
        oneProductPage['homedepot'] = 'h1.product-title__title';
        oneProductPage['asda'] = 'h1.product-name';
        oneProductPage['argos'] = 'h1.product-name-main';
        oneProductPage['pcworld'] = '.product-page h1.page-title';

        linksForProducts['overstock'] = '.product-content .pro-thumb';
        linksForProducts['wayfair'] = 'a.SbProductBlock';
        linksForProducts['kohls'] = '.products_grid .prod_img_block a';
        linksForProducts['target'] = '.details--title a';
        linksForProducts['toysrus'] = '.paraMetProductTitle a';
        linksForProducts['homedepot'] = '#products .pod-plp__brand-name';
		
        linksForProducts['asda'] = '.productName a';
        linksForProducts['argos'] = '.product-list a';
        ...
        if (isProductPage > 0)
        {
            ...
            if (thirdFoundUPCs[0]) {
              ...
              if(productName){
                var productByTitle = semantics3.query.search(productName.text());
                if(productByTitle && productByTitle.upc) {
                ...
                var productByTitleBySite = semantics3.query.searchBySite(productName.text(), localdomain+'.com');
                if(productByTitleBySite && productByTitleBySite.upc) {
        ...
        else
        {
            ...
            el.each(function(){
				...
				if (activeTab.indexOf('argos.co.uk') != -1)
                    links[links.length-1] = 'http://www.argos.co.uk'+links[links.length-1];
            });
