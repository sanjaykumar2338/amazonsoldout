	...
	init: function(table, thisWidget, c, wo) {
		...
		finishReorder = function(){
			...
			if ( s > -1 && endIndex > -1 && s != endIndex && s + 1 !== endIndex ) {
				...
				if (false) {
					console.log("adj: " + adj + " ");
					console.log("starting column: " + s + " ");
					console.log("going to column: " + endIndex + " ");
					console.log( 'Inserting column ' + s + (adj ? ' after' : ' before') + ' column ' + (endIndex - (adj ? 1 : 0)) );
				}
				target =  target - ((target <= s) ? 1 : 0);
				rows.each(function() {
					cols = $(this).children();
					cols.eq(s)[ adj ? 'insertAfter' : 'insertBefore' ]( cols.eq( target ) );
				});
				cols = [];
		...
		.add( c.$headers )
		.bind('mouseup.reorder', function(e){
			clearTimeout(timer);
			target = $($(e.target).parents("th")[0]).attr("data-column");
